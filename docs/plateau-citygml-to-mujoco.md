...
terrain GLB、道路、路面標示は、MuJoCo hfieldと同じ`BL--TR`対角線を持つ
piecewise-planar surfaceを共有します。道路と路面標示はpolygonのbboxに重なるcellだけを調べ、
各hfield triangleとのintersectionをそのtriangle平面へ配置します。既存のbilinear高さqueryは
橋梁端点診断との互換性のため分離して維持します。詳細は
[`terrain-surface-contract.md`](terrain-surface-contract.md)を参照してください。
